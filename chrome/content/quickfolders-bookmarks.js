...
QuickFolders.bookmarks = {
  ...
  findBookmark: function (entry) {
    ...
    let getFolder = QuickFolders.Model.getMsgFolderFromUri,
      ...
      searchSession = Cc["@mozilla.org/messenger/searchSession;1"].createInstance(
    ...
    if (folder) {
      ...
      offlineScope =
        folder.flags & util.FolderFlags.MSG_FOLDER_FLAG_OFFLINE
          ? nsMsgSearchScope.offlineMail
          : nsMsgSearchScope.onlineManual;
      serverScope = folder.server ? folder.server.searchScope : null;
    }
    let targetFolder =
      (preferredUri ? getFolder(preferredUri) : null) || // preferred start point for finding missing emails. could be localhost.
    ...
    let realTerm = searchSession.createTerm(),
      subj =
        entry.subject ||
        entry.label.substring(entry.label.indexOf(":") + 1, entry.label.lastIndexOf("-")),
      ellipsis = subj.indexOf("\u2026".toString());
    if (ellipsis > 5 && ellipsis >= subj.length - 2) {
      subj = subj.substr(0, ellipsis);
    }
    setTermValue(realTerm, nsMsgSearchAttrib.Subject, nsMsgSearchOp.Contains, subj.trim());
    searchTerms.push(realTerm);
    if (entry.author) {
      realTerm = searchSession.createTerm();
      let em = _getEmailAddress(entry.author);
      em = em || entry.author;
      setTermValue(realTerm, nsMsgSearchAttrib.Sender, nsMsgSearchOp.Contains, em);
      searchTerms.push(realTerm);
    }
    if (entry.date) {
      realTerm = searchSession.createTerm();
      setTermValue(realTerm, nsMsgSearchAttrib.Date, nsMsgSearchOp.Is, entry.date);
      searchTerms.push(realTerm);
    }
    window.openDialog(
  ...
  openMessage: function (entry, forceMethod) {
    ...
    try {
      ...
      if (
        msgHdr.messageId.toString() + msgHdr.author + msgHdr.subject == "" &&
        msgHdr.messageSize == 0
      ) {
        // invalid message
        return false;
      }
    ...
    try {
      switch (method) {
        case "currentTab": {
          const mode = QuickFolders.Interface.CurrentTabMode,
            // eslint-disable-next-line no-unused-vars
            _isInTab = mode == "mail3PaneTab" || mode == "folder";
      ...
      case 0:
        {
          ...
          if (isCtrl) {
            method =
              prefs.getStringPref("bookmarks.openMethod") == "newTab" ? "currentTab" : "newTab"; // accelerator toggles method
          }
          ...
          if (!this.openMessage(entry, method)) {
            ...
            let text = util.getBundleString("qf.prompt.readingList.searchMissingItem"),
              search = Services.prompt.confirm(
                window,
                "QuickFolders",
                text.replace("{1}", entry.label)
              );
            if (search) {
  ...
  addMenuItem: function (entry) {
    ...
    menuitem.addEventListener(
      "click",
      function (event) {
        QuickFolders.bookmarks.onClick(event.target, event, entry);
        return false;
      },
      false
    );
  ...
  addMail: function addMail(newUri, sourceFolder) {
    ...
    if (util.hasValidLicense() && !isStandardLicense) {
    ...
    if (countEntries >= MAX_BOOKMARKS - 2) {
      ...
      util.popupRestrictedFeature(
        "bookmarks",
        text.replace("{1}", countEntries).replace("{2}", MAX_BOOKMARKS.toString())
      );
    ...
    if (countEntries >= prefs.getIntPref("bookmarks.maxEntries")) {
      util.logToConsole(
        "Maximum number of bookmarks reached! You can change this via right-click on the Reading List checkbox in QuickFolders Options / General / Main Toolbar Elements."
      );
      return false;
    }
    ...
    for (let i = 0; i < uris.length; i++) {
      ...
      switch (uriObject.bookmarkType) {
        ...
        case "browser":
          ...
          break;
        default: {
          let msg = util.getBundleString(
            "qf.alert.bookmarks.unsupportedType",
            uriObject.bookmarkType
          );
  ...
  getBrowser: function () {
    ...
    try {
      ...
      // Try enumerating browser windows first
      let browsers = getWindowEnumerator("navigator:browser", true);
      if (!browsers || !browsers.hasMoreElements()) {
        // Fallback to mail:3pane window
      ...
      try {
        ...
        if (theBrowser.getInterface) {
          ...
          // Linux fallback
          DomWindow = theBrowser
            .QueryInterface(Ci.nsIInterfaceRequestor)
            .getInterface(interfaceType);
        }
      } catch (ex) {
        util.logException("getBrowser() failed in inner try:", ex);
      }
    } catch (ex) {
      util.logException("getBrowser() failed in outer try:", ex);
    }

    return DomWindow;
  },

  alternativeGetSelectedMessageURI: function (win) {
    try {
      ...
      console.error("alternativeGetSelectedMessageURI ex =", ex);
      return null;
    }
  },
  ...
  getActiveUri: function () {
    let util = QuickFolders.Util,
      ...
      currentType = "",
      currentFolder = null;
    if (!browser && !tabmail) {return null;} // in Linux we cannot get the browser while options dialog is displayed :(
    try {
      ...
      if (!isOriginBrowser) {
        if (tabmail) {
          ...
          if (!browser) {browser = tab.browser;}
          ...
          util.logDebugOptional(
            "bookmarks",
            "bookmarks.getActiveUri() - Selected Tab Type: " + theMode
          );
          switch (theMode) {
            case "mail3PaneTab":
              try {
            ...
            default: // case 'tasks':
              Services.prompt.alert(
                null,
                "QuickFolders.bookmarks - getActiveUri",
                "Not supported: bookmarking " + theMode + " tab!"
              );
              break;
          }
        }
  ...
  update: function () {
    ...
    QI.ReadingListButton.label = isActive ? this.Entries.length : "";
  ...
  readBookmarksFromJson: function (data) {
    let util = QuickFolders.Util,
  ...
  load: async function () {
    ...
    try {
      ...
      if (ex.becauseNoSuchFile) {
        ...
        Services.prompt.alert(
          null,
          "QuickFolders - loadCustomMenu",
          "Reading the bookmarks file failed\n" + ex
        );
