...
//Error: NS_ERROR_XPC_BAD_CONVERT_JS: Could not convert JavaScript argument arg 0 [nsIFilePicker.displayDirectory]
	...
	buildPaletteMenu: function (currentColor, existingPopupMenu, ignoreTheme, forceOnCommand) {
		...
		try {
          ...
          menuColorPopup.appendChild(menuitem);
				...
				// open "bling my tabs"
				menuitem.addEventListener("command", (event) => { QuickFolders.Interface.viewOptions(2); }, false);
				menuColorPopup.appendChild(menuitem);
			}
  ...
  folderPathLabel : function folderPathLabel(detailType, folder, maxPathItems, isUnified = false) {
	...
	addSubFoldersPopupFromList: function (
		subfolders, 
		popupMenu, 
		options = { isDrag, forceAlphaSort, isRecentFolderList }, 
		doc) 
	{
      ...
      : subfolders;
		...
		// 3 - folder path
		// 4 - folder.URI  [for debugging]
		const displayFolderPathDetail = options.isRecentFolderList
