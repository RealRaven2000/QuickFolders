...
var Register = {
    ...
    if (featureComparison) {
      ...
      window.addEventListener("click", async (event) => {
        if (event.target.id == "compLink") {
          QuickFolders.Util.openLinkInBrowser(
            event,
  ...
  load: async function () {
    ...
    document.getElementById("btnEnterCode").addEventListener("command", () => {
      QuickFolders.Interface.viewLicense();
      window.close();
    });
    ...
    // this will potentially replace some of the event handlers
    // e.g. if the license is expired.
  ...
  updateLicenseUI: async function () {
    ...
    if (decryptedDate) {
      ...
      if (licenseInfo.status == "Expired" || licenseInfo.status == "Valid") {
        const btnDomainLicense = getElement("btnDomainLicense"),
        ...
        if (licenseInfo.status == "Expired") {
          ...
          switch (licenseInfo.keyType) {
            ...
            case 2: {
              ...
              Register.addRegisterEventListener(btnStdLicense, 2, true); // extend std license
    ...
    switch (licenseInfo.status) {
      case "Expired":
        getElement("licenseDateLabel").textContent = util.getBundleString(
          "qf.register.licenseValid.expired"
        );
  ...
  updateUI: async function updateUI() {
    ...
    for (let a = 0; a < myAccounts.length; a++) {
      ...
      if (isAllowAlias && !aliasIdentity) {
        let aliasIdentity = ac.identities.find(
          (id) => id.email == QuickFolders.Util.licenseInfo.email
        );
  ...
  goPro: function (license_type, isRenew = false) {
    ...
    switch (license_type) {
      ...
      case 1: // domain license
        if (isRenew) {
          shortOrder =
            "https://sites.fastspring.com/quickfolders/instant/quickfoldersdomainrenewal";
        } else {
    ...
    const firstName = document.getElementById("firstName").value,
      ...
      url =
        `${shortOrder}?contact_fname=${firstName}${addQuery}` +
