...
QuickFolders.Interface = {
      ...
      switch (parent.id) {
        case "QuickFolders-Options-PalettePopup": {
          ...
          let paletteClass = await this.getPaletteClassCss(styleKey),
            ...
            disableColorChangeStriped =
              styleKey == "InactiveTab" && prefs.ColoredTabStyle == prefs.TABS_STRIPED;
          ...
          let selectedGradient = QuickFolders.Styles.getElementStyle(
              ssPalettes,
              ruleName,
              "background-image",
            ),
          ...
          if (styleKey == "InactiveTab" && col == 0) {
            ...
            if (chkTransparent && !chkTransparent.checked) {
              ...
              options.toggleColorTranslucent(
                chkTransparent,
          ...
          if (styleKey == "InactiveTab") {
            this.applyTabStyle(
              document.getElementById("inactivetabs-label"),
  ...
  getPaletteClass: async function (tabStateId) {
    let paletteType = await QuickFolders.Preferences.getIntPref(
      "style." + tabStateId + ".paletteType",
    );
    switch (paletteType) {
      case -1:
