...
QuickFolders.Interface = {
  ...
  get verticalMenuOffset() {
    return this._verticalMenuOffset;
  },
  set verticalMenuOffset(o) {
    this._verticalMenuOffset = o;
    QuickFolders.Preferences.setIntPref("debug.popupmenus.verticalOffset", o);
  },
  get CategoryBox() {
    return QuickFolders.Util.$("QuickFolders-Category-Box");
  },
  get FilterToggleButton() {
    return QuickFolders.Util.$("QuickFolders-filterActive");
  },
  get CogWheelPopupButton() {
    return QuickFolders.Util.$("QuickFolders-mainPopupBtn");
  },
  get QuickMoveButton() {
    return QuickFolders.Util.$("QuickFolders-quickMove");
  },
  get ReadingListButton() {
    return QuickFolders.Util.$("QuickFolders-readingList");
  },
  get CategoryMenu() {
    return QuickFolders.Util.$("QuickFolders-Category-Selection");
  },
  get PaintButton() {
    return QuickFolders.Util.$("QuickFolders-paintBucketActive");
  },
  get TitleLabel() {
    return QuickFolders.Util.$("QuickFolders-title-label");
  },
  get TitleLabelBox() {
    return QuickFolders.Util.$("QuickFolders-LabelBox");
  },
  get PreviewLabel() {
    return QuickFolders.Util.$("QuickFolders-PreviewToolbar");
  },
  get FoldersBox() {
    return QuickFolders.Util.$("QuickFolders-FoldersBox");
  },
  get Toolbar() {
    return QuickFolders.Util.$("QuickFolders-Toolbar");
  },
  get PalettePopup() {
    return QuickFolders.Util.$("QuickFolders-PalettePopup");
  },
  get FindFolderBox() {
    return QuickFolders.Util.$("QuickFolders-FindFolder");
  },
  get FindFolderWrapper() {
    return QuickFolders.Util.$("QuickFolders-FindFolder-wrapper");
  },
  get FindFolderHelp() {
    return QuickFolders.Util.$("QuickFolders-FindFolder-Help");
  },

  // CURRENT FOLDER ELEMENTS
  get CurrentFolderTab() {
    // visible current folder tab - might have to move it in Tb for conversation view
    try {
    ...
    switch (previewId) {
      ...
      default:
        QuickFolders.Util.logDebug(
          `QuickFolders.Interface.getPreviewButtonId - Invalid previewId: ${previewId}`
        );
        return null;
    }
  },

  // deprecated function to sneak in inline event handlers.
  ...
  // replacement for setEventAttribute
  addUniqueEventListener: function (element, eventType, handler) {
    if (!element._eventHandlers) {
      element._eventHandlers = new Map();
    }

    if (!element._eventHandlers.has(eventType)) {
      element.addEventListener(eventType, handler);
      element._eventHandlers.set(eventType, handler);
    }
  },

  // clean up for all added event listeners, call when tearing down popup
  removeEventListeners: function (element) {
    if (element._eventHandlers) {
      for (const [eventType, handler] of element._eventHandlers) {
        element.removeEventListener(eventType, handler);
      }
      element._eventHandlers.clear(); // Cleanup map
    }
  },
        ...
        if (entry) {
          ...
          if (entry.category) {
            util.logDebugOptional(
              "mailTabs",
              `Categories of selected Tab = ${entry.category.replace("|", ", ")}`
            );
          }
        }
      }
    } catch (e) {
      util.logToConsole("tabSelectUpdate failed: " + e);
    }
  },
    ...
    try {
      let nDelay = 250,
        tID = setTimeout(function () {
          QuickFolders.Interface.tabSelectUpdate();
        }, nDelay);
      ...
      try {
        ...
        if (item && (item.prettyName || item.localizedName)) {
          ...
          while (folderUri) {
            ...
            logDO(
              "listeners.folder",
              `Checking URI ${folderUri} returns ${entry ? entry.name : "no tab entry"}`
            );
            if (entry) {
        ...
        QI.TimeoutID = setTimeout(function () {
          QuickFolders.Interface.queuedFolderUpdate();
        }, nDelay);
        util.logDebug(`Folder Tab Select Timer ID: ${QI.TimeoutID}`);
        util.logDebug(
          `Setting Update Timer (after timer ${QI.LastTimeoutID} expired), new Timer: ${QI.TimeoutID}`
        );
  ...
  queuedFolderUpdate: function queuedFolderUpdate() {
    QuickFolders.Util.logDebugOptional(
      "listeners.folder,interface",
      `Folder Update from Timer ${this.TimeoutID}…`
    );
  ...
  createRecentPopup: function (passedPopup, isDrag, isCreate, popupId) {
    ...
    util.logDebugOptional(
      "recentFolders",
      `createRecentPopup(passedPopup: ${passedPopup}, isDrag: ${isDrag}, isCreate: ${isCreate})`
    );

    let doc =
      popupId == "QuickFolders-folder-popup-Recent-CurrentFolderTool"
        ? util.document3pane
        : document;
    ...
    if (popupId == "QuickFolders-FindFolder-popup-Recent") {
      ...
      for (let i = 0; i < QuickFolders.quickMove.history.length; i++) {
        ...
        if (f) {
          ...
          if (recentFolders.length >= maxLen) {
            break;
          }
        }
      }
    ...
    if (isProfiling) {
      ...
      console.log(
        `createRecentPopup - Before calling %caddSubFoldersPopupFromList() ${time} ms`,
    ...
    if (isProfiling) {
      ...
      console.log(
        `%cRunning createRecentPopup(${popupId}) took: ${time}`,
        "background-color: rgb(0,160,40); color:white;"
      );
    }
  ...
  createRecentTab: function (passedPopup, isDrag, passedButton) {
    try {
      ...
      let isFolderUpdate = false, //	need this to know if we are creating a fresh button (true) or just rebuild the folders menu on click/drag (false)
        isCurrentFolderButton =
          passedButton == null ? false : passedButton.id == "QuickFolders-Recent-CurrentFolderTool",
      ...
      if (!passedButton) {
        ...
        let recentLabel = QuickFolders.Preferences.getBoolPref("recentfolders.showLabel")
          ? this.getUIstring("qfRecentFolders")
          : "";
      ...
      let popupId = isCurrentFolderButton
          ? QuickFolders.Interface.RecentPopupIdCurrentFolderTool
          : QuickFolders.Interface.RecentPopupId,
      ...
      if (!isDrag) {
        ...
        if (
          !isCurrentFolderButton
        ) // the currentfolder recent button has already the correct attributes set by the overlay
        {
          if (button.getAttribute("context") != this.RecentPopupId) {
            ...
            button.addEventListener(
              "click",
              (event) => {
                QuickFolders.Interface.onClickRecent(event.target, event, true);
                return false;
              },
              false
            );
            button.addEventListener(
              "dragenter",
              (event) => {
                QuickFolders.buttonDragObserver.dragEnter(event);
              },
              false
            );
            button.addEventListener(
              "dragover",
              (event) => {
                QuickFolders.buttonDragObserver.dragOver(event);
                return false;
              },
              false
            );
  ...
  onClickThreadTools: function (button, evt) {
    ...
    if (evt.ctrlKey) {
      if (
        QuickFolders.Util.licenseInfo.status == "Valid" &&
        QuickFolders.Util.licenseInfo.keyType != 2
      ) {
  ...
  onGoNextMsg: async function (button) {
    ...
    const isSingleMessage = tabMode == "mailMessageTab";
    ...
    const isResetSearch = QuickFolders.Preferences.getBoolPref(
      "findRelated.behavior.goNextResetsSearch"
    );
    if (isResetSearch) {
  ...
  onSkipFolder: async function (_button, isSingleMessage = false) {
    ...
    if (folder) {
      util.logDebug(
        `selecting next unread folder: ${folder.prettyName || folder.localizedName}\n${folder.URI}`
      );
      //
      if (isSingleMessage) {
        ...
        if (unreadMsg) {
          console.log(`found unread message in next unread folder ${folder}`, unreadMsg);
      ...
      if (currentFolder == folder) {
        ...
        util.slideAlert(
          "QuickFolders",
          txt.replace("{1}", folder.prettyName || folder.localizedName)
        );
      }
  ...
  updateFolders: function (rebuildCategories, minimalUpdate) {
    ...
    if (profileThis) {
      ...
      console.log(
        `%cStarting updateFolders - Active Categories: ${QuickFolders.Interface.currentActiveCategories}`,
        profileStyle
      );
    }
    util.logDebugOptional(
      "interface",
      `updateFolders(rebuildCategories=${rebuildCategories}, minimalUpdate=${minimalUpdate})`
    );
    this.TimeoutID = 0;
    ...
    let sDebug = `updateFolders(rebuildCategories: ${rebuildCategories}, minimal: ${minimalUpdate})`,
      toolbar = QuickFolders.Interface.Toolbar,
    ...
    if (QuickFolders.Model.selectedFolders.length) {
      ...
      for (let i = 0; i < QuickFolders.Model.selectedFolders.length; i++) {
        ...
        if (!hasLicense) {
          if (countValidTabs > QuickFolders.Model.MAX_UNPAID_TABS) {
            // no license restriction
            button.setAttribute("disabled", true);
          }
        } else if (util.licenseInfo.keyType == 2) {
          // Standard License restriction
      ...
      let sDoneWhat = minimalUpdate
        ? "refreshed on toolbar [minimalUpdate]."
        : "rebuilt [minimalUpdate=false].";
      util.logDebug(
        `${countFolders} of ${QuickFolders.Model.selectedFolders.length} tabs ${sDoneWhat}`
      );
      if (invalidCount) {
        util.logDebug(
          `{0} invalid tabs where found!\n Please check with 'Remove Invalid tabs' from the QuickFolders tools menu.`.replace(
            "{0}",
            invalidCount
          )
        );
      ...
      // no tabs defined : add instructions label
    ...
    if (profileThis) {
      ...
      console.log(
        `%cQuickFolders.Interface.updateFolders(rebuildCat = ${rebuildCategories}, minimal = ${minimalUpdate})\n ${countFolders} folders took: ${time}`,
        profileStyle
      );
    }
  },
  ...
  updateNavigationBar: function (doc3pane, tabInfo) {
    ...
    util.logDebugOptional(
      ...
      `updateNavigationBar() - ${window.location}`,
      doc3pane
    );
    try {
      ...
      if (toolbar2) {
        ...
        if (button) {
          QuickFolders.Interface.addUniqueEventListener(button, "dragstart", (event) =>
        ...
        QuickFolders.Interface.showElement(
          skippy,
          prefs.getBoolPref("currentFolderBar.skipUnreadFolder")
        );
      ...
      case "about:3pane":
        {
          ...
          QuickFolders.Util.logDebugOptional(
            "3pane",
            `liftNavigationbar() - viewMode=${viewMode}`,
            threadPane
          );
          ...
          if (QuickFolders.Preferences.isDebug) {
            console.log(
              `[QF liftNavBar] after move — panel parent: #${navigationContainer?.parentElement?.id || "?"}, panel===in threadPane: ${navigationContainer?.parentElement === threadPane}`
            );
          }
        }
        break;
    ...
    try {
      if (lCatCount > 0 && menuList && menuPopup) {
        ...
        menuPopup.appendChild(
          this.createMenuItem(FCat.ALL, this.getUIstring("qfAll"), doc, "menuitem-iconic")
        );
        ...
        for (let i = 0; i < model.selectedFolders.length; i++) {
          ...
          if (folderEntry.category == FCat.NEVER) {
            // at least one folder alias exists
            isNever = true;
          }
          if (!folderEntry.category) {
            // at least one folder without category exists
            isUncat = true;
          }
        }
        ...
        try {
          ...
          util.getTabInfoByIndex(tabmail, idx).QuickFoldersCategory =
            QuickFolders.FolderCategory.ALL;
  ...
  deleteFolderPrompt: function (folderEntry, withCancel, check, remaining) {
    ...
    let text =
        this.getUIstring("qfThisTabIsInvalid", [folderEntry.name]) +
        "\n" +
        folderEntry.uri +
        "\n" +
        this.getUIstring("qfTabDeletePrompt"),
      checkText = check ? this.getUIstring("qfTabDeleteOption").replace("{0}", remaining) : null;

    let answer = Services.prompt.confirmEx(
      null,
      "QuickFolders",
      text,
      flags,
  ...
  tidyDeadFolders: function () {
    ...
    let activeModes = tabmail.currentAbout3Pane.folderPane.activeModes;
  ...
  repairTreeIcons: function (silent) {
    ...
    // replacement for OS.Path.fromFileURI
    const fileURIToPath = (fileURI) => {
      const url = new URL(fileURI);
      let path = url.pathname;
      // On Windows, strip leading slash
      if (QuickFolders.Util.platformInfo.os === "win" && path[0] === "/") {
        path = path.slice(1);
      }
    ...
    if (QuickFolders.FolderTree) {
      // can only work in a 3pane window!!
  ...
  get CurrentlySelectedCategories() {
    ...
    if (
      this.currentActiveCategories == FCat.ALL ||
      this.currentActiveCategories == FCat.UNCATEGORIZED
    ) {
      return null;
    } else {
  ...
  readTabCategorySession: async function (tabInfo) {
    try {
      ...
      let extTabId = QuickFolders.WL.extension.tabManager.getWrapper(tabInfo).id;
      let rv = await QuickFolders.Util.notifyTools.notifyBackground({
        func: "readCategories",
        tabId: extTabId,
      });
  ...
  storeToolbarSession: function (toolbarStatus, tabInfo) {
    let extTabId = QuickFolders.WL.extension.tabManager.getWrapper(tabInfo).id;
    QuickFolders.Util.notifyTools.notifyBackground({
      func: "storeToolbarStatus",
      tabId: extTabId,
      status: JSON.stringify(toolbarStatus),
    });
    return true;
  },
  ...
  selectCategory: function (categoryName, rebuild, dropdown, event) {
    ...
    if (multiEnabled && isShift) {
      ...
      if (currentCats.includes(selectedCat)) {
        ...
        QI.currentActiveCategories = QI.currentActiveCategories
          ? `${QI.currentActiveCategories}|${selectedCat}`
          : selectedCat;
      }
    } else {
    ...
    try {
      ...
      if (tab && ["mailMessageTab", "folder", "mail3PaneTab"].includes(tabMode)) {
    ...
    util.logDebugOptional(
      "categories",
      `Successfully selected Category: ${QI.currentActiveCategories} on mail tab [${idx}]`
    );
    return true;
  },
  ...
  shouldDisplayFolder: function (folderEntry) {
    ...
    try {
      ...
      if (!currentCat || currentCat == FCat.ALL) {
        ...
        return true;
      } else if (folderCat && folderCat == FCat.ALWAYS && currentCat != FCat.UNCATEGORIZED) {
        return true;
      } else if (!folderCat) {
        return false;
      } else {
  ...
  windowKeyPress: function (e, dir) {
    ...
    function logKey(event) {
      ...
      util.logDebugOptional(
        ...
        (isAlt ? "ALT + " : "") +
          (isCtrl ? "CTRL + " : "") +
          (isShift ? "SHIFT + " : "") +
          `key = ${event.key} = ${event.key.toLowerCase()}\n` +
    ...
    if (QuickFolders.Util.lastTime) {
      ...
      if (elapsed < 220 && e.key === QuickFolders.Util.lastKey) {
        util.logDebugOptional(
          "events.keyboard",
          `discarding repeat key ${e.key} after ${elapsed}ms`,
          e
        );
          ...
          if (
            ...
            if (QuickMove.isActive && QuickMove.hasMails) {
              if (!QuickMove.suspended) {
                QuickMove.toggleSuspendMove();
              } // suspend move to jump; also reflect in menu.
  ...
  getButtonByFolder: function (folder) {
    for (let i = 0; i < this.buttonsByOffset.length; i++) {
      ...
      try {
        ...
        QuickFolders.Util.logDebug(
          `getButtonByFolder: could not match ${button.folder.URI} error: ${e}`
        );
      }
    }
  ...
  toggleToolbar: async function (
    toggleOptions = { button: null, toggle: true, forceVisible: null }
  ) {
    QuickFolders.Util.logDebugOptional("interface", "toggleToolbar()", toggleOptions);
    const toolbar = QuickFolders.Interface.Toolbar,
    ...
    let isPreviewMode = toggleOptions.optionsMode || false;
    if (
      QuickFolders.Interface.CurrentTabMode &&
  ...
  showPopup: function (button, popupId, evt, noCommands) {
    ...
    if (evt) {
      ...
      util.logDebugOptional(
        "interface,popupmenus",
        `showPopup("${button.id}", "${popupId}", NO EVENT)`
      );
    }
      ...
      if (evt && evt.ctrlKey) {
        ...
        if (menupopup) {
          ...
          if (mps.length) {
            ...
            QI.addUniqueEventListener(mps[0], "click", (event) => {
              event.stopPropagation();
              QuickFolders.Interface.clickHandler(event, mps[0]);
            });
            QI.addUniqueEventListener(mps[0], "command", (event) => {
              event.stopPropagation();
              QuickFolders.Interface.clickHandler(event, mps[0]);
            });
          }
          ...
          for (let i = 0; i < nodes.length; i++) {
            if (nodes[i].id == "quickFoldersMailFolderCommands") {
              if ("quickFoldersMailFolderCommands" != menupopup.firstChild.id) {
                // [issue 255] avoid duplication
  ...
  _addCountToDisplay: function (displayNumbers, numCount, numSubfolderCount, showSubfolderCount) {
    ...
    if (numSubfolderCount > 0 && showSubfolderCount) {
      s += triangleDown + numSubfolderCount;
    }
    if (s !== "") {
  ...
  _createAriaLabel: function (
    ...
    if (isShowUnread) {
      if (numUnread) {
        ariaLabel += `, ${getBS("accessibility.count.unread", numUnread)}`;
      }
    ...
    if (numTotal > 0 || numTotalInSubFolders > 0) {
      if (numTotal) {
        ariaLabel += `, ${getBS("accessibility.count.total", numTotal)}`;
      }
  ...
  getButtonLabel: function (folder, useName, offset, entry, stats) {
    ...
    try {
      ...
      const isShowTotals = this._shouldShowCounts(
        entry,
        prefs.isShowTotalCount,
        util.ADVANCED_FLAGS.SUPPRESS_COUNTS
      );
      const isShowUnread = this._shouldShowCounts(
        entry,
  ...
  addFolderButton: function (
    ...
    const doc = theButton
      ? theButton.ownerDocument
      : buttonId == "QuickFoldersCurrentFolder"
        ? util.document3pane
        : document;

    if (!folder && !isMinimal) {
      util.logToConsole(
        "Error in addFolderButton: folder parameter is empty!\n" +
    ...
    if (!theButton) {
      isMinimal = false;
    } // if we create new buttons from scratch, then we need a full creation including menu

    util.logDebugOptional(
      "interface.tabs",
      `addFolderButton() label=${label}, offset=${offset}, col=${tabColor}, id=${buttonId}, fillStyle=${fillStyle}`
    );
    ...
    if (folder) {
      ...
      } else if (folder.flags & FLAGS.MSG_FOLDER_FLAG_ARCHIVES) {
        // Dillinger Bug
      ...
      button.addEventListener(
        ...
        (event) => {
          if (event.target === button) {
            // [issue 541]
            event.stopPropagation();
            QuickFolders.Interface.onButtonClick(event.target, event, true);
          }
        },
        false
      );
      ...
      if (!button.hasContextListener) {
        ...
        button.addEventListener(
          ...
          (event) => {
            ...
            QuickFolders.Interface.showPopup(button, pId, event);
      ...
      if (buttonId == "QuickFoldersCurrentFolder") {
        if (!button.hasClickEventListener) {
          ...
          button.addEventListener(
            ...
            (event) => {
              ...
              if (event.button == 0) {
                ...
                let popupId = QuickFolders.Interface.makePopupId(event.target.folder, buttonId);
                QuickFolders.Interface.showPopup(button, popupId, event);
                event.preventDefault();
                event.stopPropagation();
              }
            },
          ...
          QI.addUniqueEventListener(button, "dragleave", (event) =>
            QuickFolders.buttonDragObserver.dragLeave(event, true)
          );
        }
      }
    }
    ...
    if (!isMinimal) {
      ...
      setTimeout(
        // make this async
        function () {
          QuickFolders.Interface.addPopupSet({
            popupId: popupId,
            folder: folder,
            entry: entry,
            offset: offset,
  ...
  addCustomStyles: function (button, entry) {
    const util = QuickFolders.Util,
    ...
    if (entry && entry.flags && entry.flags & ADVANCED_FLAGS.CUSTOM_CSS) {
      try {
        ...
        QuickFolders.Util.logException("custom CSS failed", ex.toString());
      }
      return;
    }
  ...
  onButtonClick: function (button, evt, isMouseClick) {
    ...
    util.logDebugOptional("mouseclicks,accessibility", "onButtonClick()", {
      isMouseClick,
      isEnterKey,
      evt,
    });
    ...
    try {
      if (QI.PaintModeActive) {
        ...
        if (evt.ctrlKey) {
          // go to next / previous color! (RAINBOW MODE)
          if (evt.shiftKey) {
      ...
      if (evt) {
        ...
        if (evt.ctrlKey && isMouseClick) {
          if (evt.button == 0) {
            // only main mouse button (right-click) is shortcut for commands menu
            ...
            util.logDebug(`${evt?.type} event on button, passed through from : ${evt.targetNode}`);
          }
        }
      }
    } catch (ex) {
  ...
  onRemoveBookmark: function (element) {
    ...
    if (folder) {
      ...
      // let's store folder URI for invalid loaded tabs
      QuickFolders.Model.removeFolder(tab.getAttribute("folderURI"), true);
    }
  ...
  onRemoveIcon: function (element, event) {
    ...
    if (
      element.id == "context-quickFoldersRemoveIcon" || // folder tree icon
      element.id == "QuickFolders-RemoveIcon"
    ) {
      // current folder bar
      ...
      switch (element.id) {
        case "QuickFolders-RemoveIcon": // current folder bar
          ...
          break;
        case "context-quickFoldersRemoveIcon":
          {
            // style a folder tree icon - retrieve folder from event.detail
            util.logDebugOptional("folderTree.icons", event.detail);
            const fld = QuickFolders.Model.getMsgFolderFromUri(event.detail?.folderURI);
            folders = fld ? [fld] : [];
          }
          break;
      }
  ...
  onSelectIcon: function (element, event) {
    ...
    if (isRestricted && countIcons >= maximumIcons) {
      let text = util.getBundleString("qf.notification.premium.tabIcons", [
        countIcons,
        model.MAX_UNPAID_ICONS,
        model.MAX_STANDARD_ICONS,
      ]);
        ...
        try {
          ...
          if (folders) {
            for (let i = 0; i < folders.length; i++) {
              ...
              if (entry) {
                // if button visible, update it!
                folderButton = QuickFolders.Interface.shouldDisplayFolder(entry)
                  ? QuickFolders.Interface.getButtonByFolder(folder)
                  : null;
  ...
  onRenameBookmark: function (element) {
    ...
    if (QuickFolders.Preferences.isShowTotalCount || QuickFolders.Preferences.isShowUnreadCount) {
      ...
      if (i > 1 && sOldName.substr(i, j - i).length > 0 && j == sOldName.length - 1) {
        ...
        util.logDebug(
          `Suspected number of new / total mails = ${sOldName.substr(i, j - i + 1)}, length = ${bracketedLen}`
        );
    ...
    let fUri = folderButton.folder
      ? folderButton.folder.URI
      : folderButton.getAttribute("folderURI");
  ...
  onAdvancedProperties: function (evt, element) {
    ...
    if (isHTMLprops) {
      ...
      if ((!isLinuxPlatform && !isMacPlatform) || forcePopup) {
        // avoid color popups being hidden on Mac
        winProps += ",popup=yes";
      }
      if (forceRaised) {
  ...
  compactFolder: function (folder, command) {
    ...
    if (!folder) {
      QuickFolders.Util.alertButtonNoFolder(null);
      return;
    }
  ...
  onMarkAllRead: async function (element, evt, recursive) {
    ...
    try {
      ...
      if (recursive) {
        // [issue 3] Mark messages READ in folder and all its subfolders
  ...
  onMarkAllReadNewsGroup: function (element, _evt) {
    const util = QuickFolders.Util,
      Ci = Components.interfaces,
      folder = util.getPopupNode(element).folder,
    ...
    if (folder.flags & Ci.nsMsgFolderFlags.Virtual) {
      topChromeWindow.MsgMarkAllRead(VirtualFolderHelper.wrapVirtualFolder(folder).searchFolders);
    } else {
  ...
  onDeleteJunk: function (element) {
    ...
    if (!folder.getFlag(Ci.nsMsgFolderFlags.Virtual)) {
      let junkMsgHdrs = [];
      for (let msgHdr of folder.messages) {
        // gDBView.msgFolder.messages
  ...
  openExternal: function (aFile) {
    ...
    try {
      ...
      let uri = Cc["@mozilla.org/network/io-service;1"]
          ...
          .newFileURI(aFile),
        protocolSvc = Cc["@mozilla.org/uriloader/external-protocol-service;1"].getService(
  ...
  onGetMessages: function (element) {
    ...
    if (!folder) {
      util.logToConsole("could not retrieve folder from menu / button: ", {
        element: element,
        popupNode: btn,
        folder: folder,
      });
    ...
    if (
      folder.flags & util.FolderFlags.MSG_FOLDER_FLAG_NEWSGROUP ||
  ...
  rebuildSummary: async function (folder) {
    ...
    QuickFolders.Util.logDebug(
      `rebuildSummary(${folder.prettyName || folder.localizedName}) started... `
    );
    ...
    if (folder.supportsOffline) {
      ...
      await IOUtils.remove(folder.filePath.path, { recursive: true }).catch(console.error);
    }
    ...
    QuickFolders.Util.slideAlert(
      "QuickFolders",
      this.getUIstring("qfFolderRepairedMsg") + " " + (folder.prettyName || folder.localizedName)
    );
  },
  ...
  onCreateInstantFolder: async function (parentFolder, folderName) {
    ...
    if (!parentFolder.canCreateSubfolders) {
      throw (
        "cannot create a subfolder for: " + (parentFolder.prettyName || parentFolder.localizedName)
      );
    }
  ...
  buildPaletteMenu: function (currentColor, existingPopupMenu, ignoreTheme, forceOnCommand) {
    ...
    let menuColorPopup = existingPopupMenu
      ? existingPopupMenu
      : this.createIconicElement("menupopup", "*");
    try {
      ...
      if (
        ...
        for (let jCol = 0; jCol <= 20; jCol++) {
          ...
          if (forceOnCommand) {
            QI.addUniqueEventListener(menuitem, "command", (event) =>
              QI.setTabColorFromMenu(event.target, jCol)
            );
          }
        ...
        menuitem.addEventListener(
          "command",
          (_event) => {
            QuickFolders.Interface.viewOptions(2);
          },
          false
        );
        menuColorPopup.appendChild(menuitem);
  ...
  appendMailFolderCommands: function (MailCommands, folder, isRootMenu, button, menupopup) {
    ...
    // Empty Trash
    if (
      folder.flags & util.FolderFlags.MSG_FOLDER_FLAG_TRASH &&
    ...
    if (
      ...
      menuitem = createMailCmdMenuItem(
        "QF_folderPaneContext-getMessages",
        this.getUIstring("qfGetMail")
      );
    ...
    if (menupopup && folder) {
      ...
      try {
        ...
        if (srv) {
          let type = srv.type;
          if (
            type !== "nntp" && // newsgroups have their own "Get Messages" Command
            type !== "pop3" &&
        ...
        util.logException(
          `Exception in appendMailFolderCommands for ${folder.name} this should have a server property!`,
          ex
        );
      }
    ...
    if (prefs.getBoolPref("folderMenu.emptyJunk")) {
      ...
      if (folder.flags & util.FolderFlags.MSG_FOLDER_FLAG_JUNK) {
        ...
        !(
          folder.flags &
          (util.FolderFlags.MSG_FOLDER_FLAG_TRASH | util.FolderFlags.MSG_FOLDER_FLAG_NEWSGROUP)
        ) &&
    ...
    if (folder.canCreateSubfolders) {
      ...
      menuitem.addEventListener(
        ...
        false
      ); // menuitem instead of this (represent the calling element)
    ...
    // Repair Folder
    menuitem = createMailCmdMenuItem(
      "quickFoldersFolderRepair",
      this.getUIstring("qfFolderRepair"),
      "qfFolderRepair"
    );
    ...
    let srchMenu = util
      .getMail3PaneWindow()
      .document.getElementById("QF_folderPaneContext-searchMessages");
    let searchLabel = srchMenu
      ? srchMenu.getAttribute("label")
      : this.getUIstring("qfFolderSearchMessages") + "…";
    menuitem = createMailCmdMenuItem(
      "quickFolders-folderSearchMessages",
      searchLabel,
      "qfFolderSearch"
    );
    if (srchMenu) {
      ...
      if (ak) {
        menuitem.setAttribute("accesskey", ak);
      }
    }
    MailCommands.appendChild(menuitem);
    ...
    // Folder Properties
    menuitem = createMailCmdMenuItem(
      "QF_folderPaneContext-properties",
      this.getUIstring("qfFolderProperties")
    );
    ...
    MailCommands.appendChild(this.createIconicElement("menuseparator", "*", doc));
    menuitem = createMailCmdMenuItem(
      "quickFolders-openFolderLocation",
      this.getUIstring("qfFolderOpenLocation")
    );
    MailCommands.appendChild(menuitem);
  },
  ...
  buildQuickFoldersCommands: function (vars) {
    ...
    if (prefs.getBoolPref("commandMenu.options")) {
      cloneItem("QuickFolders-ToolbarPopup-options", () => QuickFolders.Interface.viewOptions(-1));
    }

    // Support
    if (prefs.getBoolPref("commandMenu.support")) {
      cloneItem("QuickFolders-ToolbarPopup-support", () => QuickFolders.Interface.viewSupport());
    }
  ...
  checkIsDuplicateEvent: function (o) {
    ...
    try {
      ...
      util.logException("checkIsDuplicateEvent", ex);
    } finally {
  ...
  clickHandler: function (evt, element) {
    ...
    if (evt.target) {
      ...
      if (tag) {
        ...
        while (par && par.parentNode && par.tagName != "toolbarbutton") {
          par = par.parentNode;
        }
      ...
      if (id && !isTagHandler) {
        // include case: if both were given but tag handler didn't do it.
        if (QI.checkIsDuplicateEvent({ id: id })) {
          return;
        }
        switch (id) {
          case "QF_folderPaneContext-openNewTab":
            {
              const folder = QuickFolders.Util.CurrentFolder;
              QI.openFolderInNewTab(folder);
            }
            break;
  ...
  createMenuItem_disabled: function () {
    ...
    const txt = util.hasValidLicense()
      ? util.getBundleString("license_restriced.standard.maxtabs", [
          QuickFolders.Model.MAX_STANDARD_TABS,
        ])
      : util.getBundleString("license_restriced.unpaid.maxtabs", [
          QuickFolders.Model.MAX_UNPAID_TABS,
        ]);
  ...
  attachPopupListener: function (menupopup) {
    ...
    menupopup.addEventListener("popupshowing", (_e) => {
      ...
      menupopup
        ...
        .forEach((mp) => QuickFolders.Interface.attachPopupListener(mp));
    });
  },
  ...
  addPopupSet: function (popupSetInfo) {
    ...
    if (isProfiling) {
      QuickFolders.Util.stopWatch("reset", "addPopupSet");
    }
    ...
    if (folder) {
      ...
      util.logDebugOptional(
        "popupmenus",
        `Creating Popup Set for ${fi.prettyName || fi.localizedName}\nId: ${popupId}\nuri: ${fi.URI}\nflags: ${fi.flags}`
      );
    ...
    if (isProfiling) {
      console.log(
        `Creating child folders submenu for ${popupId} took:\n` +
    ...
    if (showCommandsSubmenus) {
      ...
      if (folder && !isUnifiedFolder) {
        ...
        if (prefs.getBoolPref("folderMenu.openNewTab")) {
          ...
          let label = newTabMenuItem?.label
            ? newTabMenuItem.label.toString()
            : util.getBundleString("qf.label.menus.openInNewTab");
          ...
          menuitem.addEventListener(
            ...
            (_event) => {
              if (
                QuickFolders.Interface.checkIsDuplicateEvent({
                  tag: "openNewTab",
                  id: "QF_folderPaneContext-openNewTab",
                })
              ) {
                return;
              }
              QuickFolders.Interface.openFolderInNewTab(fi);
            },
            true
          );
          menupopup.appendChild(menuitem);
        ...
        if (topShortCuts > 0 && fi.hasSubFolders) {
          // separator only if necessary
      ...
      if (button.id != "QuickFoldersCurrentFolder" && showCommandsSubmenus) {
        ...
        if (qfCmdsMenu) {
          menupopup.appendChild(qfCmdsMenu);
        }
      }
    ...
    if (fi && fi.hasSubFolders) {
      ...
      if (!isDisabled) {
        menupopup.setAttribute("toplevel", true); // avoid the extra menu item of accessibility.subFolderParentEntry
        this.addSubFoldersPopup(menupopup, folder, false);
      }
      util.logDebugOptional(
        "popupmenus",
        `Created Menu ${folder.name}: ${this.debugPopupItems} items.\n-------------------------`
      );
      ...
      if (isUnifiedFolder) {
        let folder = smartRoot.getChildWithURI(`${smartRoot.URI}/${specialName}`, false, true);
        if (folder) {
          let FoldersArray = [];
          for (let server of MailServices.accounts.allServers) {
            for (let f of server.rootFolder.getFoldersWithFlags(folderFlag)) {
              FoldersArray.push(f);
            }
          }
          const popupOptions = {
            ...
            isRecentFolderList: false,
            isUnifiedFolder: isUnifiedFolder,
          };
          this.addSubFoldersPopupFromList(FoldersArray, menupopup, popupOptions, doc);
        }
      }
    }
    ...
    if (isProfiling) {
      ...
      console.log(
        `%cComplete FUNCTION addPopupSet(${pId}) took: %c${time}%c\n===============`,
        popupSetInfo,
  ...
  showCurrentFolderMailContextMenu: function (button) {
    ...
    QI.addUniqueEventListener(menupopup, "click", (event) => {
      event.stopPropagation();
      QuickFolders.Interface.clickHandler(event, event.target);
    });
    QI.addUniqueEventListener(menupopup, "command", (event) => {
      event.stopPropagation();
      QuickFolders.Interface.clickHandler(event, event.target);
  ...
  createMenuItem_GetMail: function (folder, doc) {
    ...
    try {
      ...
      QuickFolders.Util.logException(
        "Exception in createMenuItem_GetMail (Get Mail Command for Inbox): " + server,
        ex
      );
      return null;
    }
  ...
  createMenuItem_MarkAllRead: function (disabled, doc, recursive = false) {
    ...
    menuitem.setAttribute(
      "label",
      recursive ? this.getUIstring("qfMarkAllReadRecursive") : this.getUIstring("qfMarkAllRead")
    );
    if (!recursive) {
  ...
  createMenuItem_MarkAllRead_NewsGroup: function (disabled, doc) {
    // [issue 458]
  ...
  addSubMenuEventListener: function (subMenu, url) {
    ...
    // [issue 560]
    /*  this doesn't work, key events go on lower level
		subMenu.addEventListener(
      "keydown",
      function (evt) {
  ...
  addDragToNewFolderItem: function (popupMenu, folder) {
    const util = QuickFolders.Util,
      ...
      Ci = Components.interfaces;
    try {
      if (typeof folder.server === "undefined") {
        return;
      }

      util.logDebugOptional(
        "dragToNew",
        `addDragToNewFolderItem	${folder.prettyName || folder.localizedName}` +
        ...
        default:
          try {
            if (!prefs.getBoolPref("dragToCreateFolder." + server.type)) {
              util.logDebugOptional(
                "dragToNew",
                `Not enabled: drag & create new folder for server's of type: ${server.type}`
              );
              return;
            }
          } catch {;}
      ...
      if (folderPaneContext) {
        ...
        if (newMenuItem) {
          ...
          // use parent folder URI as each starting point
          QI.addUniqueEventListener(createFolderMenuItem, "dragenter", (event) =>
            QuickFolders.popupDragObserver.dragEnter(event)
          );

          QI.addUniqueEventListener(createFolderMenuItem, "drop", (event) =>
          ...
          QI.addUniqueEventListener(createFolderMenuItem, "drop", (event) =>
            // only case where we use the dedicated observer of the popup!
            QuickFolders.popupDragObserver.drop(event)
          );
      ...
      util.logException("Exception in addDragToNewFolderItem (adding drag Menu items): ", ex);
    }
  },
  ...
  folderPathLabel: function (detailType, folder, maxPathItems, isUnified = false) {
    ...
    switch (detailType) {
      ...
      case 2: {
        // account name - folder path (max[n])
        hostString = folder.rootFolder.name;
  ...
  // options.isDrag: if this is set to true, then the command items are not included
  addSubFoldersPopupFromList: function (
    subfolders,
    popupMenu,
    ...
    if (folder.hasSubFolders) {
      ...
      if (isProfiling) {
        ...
        const fName = folder
          ...
          : "[missing folder]";
        console.log(
          `%caddSubFoldersPopup(${fName}) - Creating addSubFoldersPopupFromList() took: ${time}`,
          "background-color: rgb(0,160,40); color:white;"
        );
      }
    }
  ...
  collapseParentMenus: function (Target) {
    ...
    util.logDebugOptional(
      "popupmenus.collapse",
      `Close menus for node=${p.nodeName}` +
      ...
      case "menu": // clicked on a parent folder?
        ...
        while (null != p.parentNode && p.tagName != "toolbar") {
          ...
          if (p.tagName == "menupopup" && p.hidePopup) {
            util.logDebugOptional(
              "popupmenus.collapse",
              `Try hide parent Popup ${p.getAttribute("label")}`
            );
            p.hidePopup();
          }
        }
      ...
      case "toolbarbutton":
        {
          ...
          util.logDebugOptional(
            "popupmenus.collapse",
            `set QuickFolders_globalHidePopupId to ${QuickFolders_globalHidePopupId}`
          );
          ...
          if (popup) {
            try {
              ...
              util.logDebugOptional(
                "popupmenus.collapse",
                `Could not remove popup of ${QuickFolders_globalHidePopupId}`
              );
            }
          }
        }
  ...
  onSelectSubFolder: function (folderUri, evt) {
    ...
    util.logDebugOptional(
      "interface,popupmenus",
      `onSelectSubFolder: ${folderUri} ${evt ? `\n type= ${evt.type} \n target= ${evt.target}` : "[no event argument]"}`
    );
    if (evt) {
      evt.preventDefault();
      evt.stopPropagation();
  ...
  hideFindPopup: function () {
    ...
    util.logDebugOptional(
      ...
      `hideFindPopup - menupopup status = ${state}`
    );
  ...
  findFolderName: async function (searchBox, options = { forceFind: false }) {
    ...
    function buildParentString(folder, parentCount) {
      ...
      if (parentString) {
        ...
        while (parts.length && par.parent) {
          ...
          if ((par.prettyName || par.localizedName).toLocaleLowerCase().startsWith(p)) {
            p = parts.pop();
          }
          par = par.parent;
        }
    ...
    function addMatchingFolder(matches, folder) {
          ...
          for (let i = 0; i < fArray.length; i++) {
            if (fArray[i].startsWith(search)) {
              if (search == fArray[i]) {
                ...
                rank += 3;
              } // we could also use the length of the string for ranking? or search.length / fArray[i].length  as a percentage
      ...
      if (searchFolderName == "" && parentString != "") {
        matchPos = 0;
      }
      if (matchPos >= 0) {
        ...
        if (
          ...
          if (searchFolderName.length == (folder.prettyName || folder.localizedName).length) {
            rank += 7;
          } // full match - promote
          if (matchPos == 0) {
            rank += 3;
    ...
    function payLoad(finalURI, options) {
      ...
      if (!QuickFolders.quickMove.isActive) {
        ...
        let ps = parentCount ? buildParentString(targetFolder, parentCount) : "";
        setTimeout(function () {
  ...
  selectFound: function (element, event) {
    ...
    if (el.classList.contains("deferred")) {
      return;
    }
    element.setAttribute("ignorekeys", "true");
    ...
    if (isQuickMove) {
      ...
      if ((isForceTab && !event.ctrlKey) || (event.ctrlKey && !isForceTab)) {
        // XOR
    ...
    if (isSelected) {
      ...
      if (el.classList.contains("quickFolder")) {
        ...
        // this should not happen as we have found it from the folder tree!
  ...
  correctFolderEntry: function (URI) {
    let confirmationText =
        "could not find this QuickFolder! The URL might be invalid - this can be caused by moving parent folders.\n" +
  ...
  setFocusTabs: function () {
    ...
    if (!button) {
      ...
      if (idx >= QI.buttonsByOffset.length) {
        // no eligible buttons
        return;
      }
  ...
  getCurrentTabMailFolder: function () {
    ...
    if (tabMode == "mailMessageTab") {
      ...
      if (folder) {
        QuickFolders.Util.logDebugOptional(
          "mailTabs",
          `getCurrentTabMailFolder() returns displayed message folder: ${folder.prettyName || folder.localizedName}`
        );
        return folder;
      }
    }
    let fD = info.folderDisplay;
    if (fD && fD.view && fD.view.displayedFolder) {
      // Tb
    ...
    QuickFolders.Util.logDebugOptional(
      "mailTabs",
      `getCurrentTabMailFolder() returns: ${folder ? folder.prettyName || folder.localizedName : "n/a"}`
    );
    return folder;
  },
  ...
  onTabSelected: function (forceButton, forceFolder) {
    ...
    if (null == folder) {
      return null;
    } // cut out lots of unneccessary processing!
    ...
    for (let i = 0; i < QI.buttonsByOffset.length; i++) {
      ...
      if (
        ...
        button.className.indexOf("striped") < 0
      ) {
      ...
      if (button.folder) {
        if (showNewMail) {
          ...
          } else if (button.folder.hasNewMessages) {
            // is https://developer.mozilla.org/en/XPCOM_Interface_Reference/nsMsgFolderFlagType lying? My inbox has 80001004, which is
            button.setAttribute("biffState-NewMail", "true");
          }
        }
  ...
  hoistCurrentFolderBar: function (currentFolderTab, tabInfo) {
    ...
    util.logDebugOptional(
      "interface.currentFolderBar,3pane",
      `hoistCurrentFolderBar(tabMode: ${tabMode})`
    );
...
*/
  ...
  initCurrentFolderTab: function (currentFolderTab, folder, selectedButton, tabInfo) {
    ...
    try {
      ...
      if (folder) {
        ...
        QuickFolders.Interface.addFolderButton(
          folder,
          entry,
          -1,
          currentFolderTab,
          "QuickFoldersCurrentFolder",
          prefs.ColoredTabStyle,
          true,
          false
        );
        ...
        QuickFolders.Interface.showElement(CurrentFolderFindRelatedBtn, prefs.supportsFindRelated);
        if (QuickFolders.FolderTree && CurrentFolderRemoveIconBtn) {
          if (!prefs.supportsCustomIcon) {
            ...
            let hasIcon = prefs.getBoolPref("currentFolderBar.folderTreeIcon")
        ...
        setTimeout(() => {
          const tabmail = document.getElementById("tabmail"),
            idx = QuickFolders.tabContainer.tabbox.selectedIndex || 0,
            tabInfo = QuickFolders.Util.getTabInfoByIndex(tabmail, idx);
          currentFolderTab.setAttribute("label", tabInfo?.title || "?");
        }, 250);

        if (tabMode == "glodaList") {
          // add search icon!
          currentFolderTab.style.listStyleImage =
            "url('chrome://global/skin/icons/Search-glass.png')";
  ...
  configureCategory: function (folder, quickfoldersPointer) {
    ...
    window.openDialog(
      "chrome://quickfolders/content/set-folder-category.xhtml",
      "quickfolders-set-folder-category",
      "chrome,titlebar,toolbar,centerscreen,modal=no,resizable,alwaysRaised",
      quickfoldersPointer,
      folder,
      retval
    );
  ...
  removeFromCategory: function (element) {
    const folderButton = QuickFolders.Util.getPopupNode(element),
      entry = QuickFolders.Model.getButtonEntry(folderButton),
      cats = entry.category.split("|");
    let removeAlwaysShow = false;
    // see if it is set to "show always" and show a prompt allowing to remove
    if (cats.includes(QuickFolders.FolderCategory.ALWAYS)) {
    ...
    const newCategories = [];
    for (const category of cats.map((c) => c.trim())) {
      // iterate + trim
      if (category === this.currentActiveCategories.trim()) {
        continue;
      }
      if (removeAlwaysShow && category === QuickFolders.FolderCategory.ALWAYS) {
        continue;
      }
      newCategories.push(category);
    }

    entry.category = newCategories.join("|");
    QuickFolders.Model.update();
  },
  ...
  getButtonColorClass: function (col, disableStriped) {
    ...
    return `col${col}${tabStyle == QuickFolders.Preferences.TABS_STRIPED && !disableStriped ? "striped" : ""}`;
  },
  ...
  initElementPaletteClass: function (element, targetElement, isUncolored) {
    ...
    let paletteToken = isUncolored
      ? this.getPaletteClass("InactiveTab")
      : this.getPaletteClass("ColoredTab"); // QuickFolders.Preferences.isPastelColors;
    ...
    QuickFolders.Util.logDebugOptional(
      "css.palette",
      `initElementPaletteClass(element: ${element?.id || element?.tagName}\ntarget: ${
        targetElement?.label || targetElement?.tagName || "none"
  ...
  setPaintButtonColor: function (col) {
    ...
    // striped
    if (
      QuickFolders.Preferences.ColoredTabStyle == QuickFolders.Preferences.TABS_STRIPED &&
      paintButton.className.indexOf("striped") < 0
    ) {
  ...
  applyTabStyle: function (el, styleId) {
    if (!el) {
      return;
    }
  ...
  getStyleSheet: function (doc, Name, Title) {
    ...
    if (!sheet) {
      QuickFolders.Util.logToConsole(
        `ensureStyleSheetLoaded() - missing style sheet '${Name}' - not found\n not attempting any style modifications.`,
        doc
      );
    }
    return sheet;
  ...
  initHoverStyle: function (ss, ssPalettes, isPaintMode) {
    ...
    QuickFolders.Util.logDebugOptional(
      ...
      `initHoverStyle()  PaintMode=${isPaintMode}   paletteClass=${paletteClass}`
    );
    ...
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton:hover",
      "background-color",
      hoverBackColor,
      true
    );
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton." + noColorClass + ":hover",
      "background-color",
      hoverBackColor,
      true
    );
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton." + noColorClass + ":hover .toolbarbutton-text",
      "color",
      hoverColor,
      true
    ); // [issue 81] - add selector for label
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton." + noColorClass + ":hover .toolbarbutton-icon",
      "color",
      hoverColor,
      true
    ); // [issue 81] - add selector for icon
    ...
    if (prefs.getIntPref("style.HoveredTab.paletteType") || isPaintMode) {
      ...
      engine.setElementStyle(
        ss,
        '.quickfolders-flat toolbarbutton[buttonover="true"]',
        "color",
        hoverColor,
        true
      );
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton" + paletteClass + ":hover",
        "background-image",
        "none",
        true
      );
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton" + paletteClass + "." + noColorClass + ":hover",
        "background-image",
        "none",
        true
      );
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton." + noColorClass + '[buttonover="true"]',
        "color",
        hoverColor,
        true
      );
  ...
  initDragOverStyle: function (ss, ssPalettes) {
    ...
    engine.setElementStyle(
      ...
      "background-color",
      prefs.getUserStyle("DragTab", "background-color", "#E93903"),
      true
    );
    let noColorClass = "col0"; // ####
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton." + noColorClass + ".dragover",
      "color",
      dragOverColor,
      true
    ); // ####
    ...
    if (prefs.getIntPref("style.DragOver.paletteType")) {
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton" + paletteClass + ".dragover",
        "color",
        dragOverColor,
        true
      );
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton" + paletteClass + '[buttonover="true"]',
        "color",
        dragOverColor,
        true
      );
  ...
  initSelectedFolderStyle: function (ss, ssPalettes, _tabStyle) {
    ...
    const engine = QuickFolders.Styles,
      colActiveBG = QuickFolders.Preferences.getUserStyle(
        "ActiveTab",
        "background-color",
        "Highlight"
      ),
    ...
    if (QuickFolders.Preferences.getIntPref("style.ActiveTab.paletteType")) {
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton.selected-folder",
        "background-color",
        colActiveBG,
        true
      );
  ...
  initDefaultStyle: function (ss, ssPalettes, tabStyle) {
    ...
    let engine = QuickFolders.Styles,
      ...
      inactiveBackground = util.getSystemColor(
        prefs.getUserStyle("InactiveTab", "background-color", "ButtonFace")
      ),
    ...
    const activeTabLineColor = tabLine
      ? getComputedStyle(tabLine).backgroundColor
      : "var(--tabline-color)";
    ...
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton#QuickFoldersCurrentFolder",
      "background-color",
      inactiveBackground,
      true
    );
    ...
    if (prefs.getIntPref("style.InactiveTab.paletteType") > 0) {
      ...
      const ruleName =
        (!areTabsStriped ? ".quickfolders-flat " : "") + paletteClass + ".col" + paletteEntry;
      ...
      engine.removeElementStyle(
        ss,
        `.quickfolders-flat toolbarbutton.${noColorClass}:not(.dragover)`,
        "background-image"
      ); // remove "none"
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat ." + noColorClass + ":not(.dragover)#QuickFoldersCurrentFolder",
        "background-image",
        inactiveGradient,
        false
      );
      ...
      inactiveGradientColor =
        inactiveColor == "black"
          ? engine.getElementStyle(ssPalettes, ruleName, "color")
          : inactiveColor;
    } else {
      engine.setElementStyle(
        ss,
    ...
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton" + "." + noColorClass + " .toolbarbutton-text",
      "color",
      inactiveColor,
      false
    ); // [issue 81] - add selector for label
    engine.setElementStyle(
      ss,
      ".quickfolders-flat toolbarbutton" + "." + noColorClass + " .toolbarbutton-icon",
      "color",
      inactiveColor,
      false
    ); // [issue 81] - add selector for icon
    if (inactiveGradientColor != null) {
      engine.setElementStyle(
        ss,
        `.quickfolders-flat toolbarbutton${paletteClass}.${noColorClass}`,
        "color",
        inactiveGradientColor,
        false
      );
    ...
    if (areTabsStriped) {
      // paletteClass = plastic + pastel only
      ...
      engine.setElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton#QuickFoldersCurrentFolder.col0" + paletteClass,
        "color",
        inactiveColor,
        false
      );
      ...
      engine.removeElementStyle(
        ss,
        ".quickfolders-flat toolbarbutton" + coloredPaletteClass,
        "color"
      );
    }
  },
  ...
  updateUserStyles: function () {
    ...
    try {
      util.logDebugOptional(
        "interface",
        `updateUserStyles() ${window.location}\nTheme: ${prefs.CurrentThemeId}`
      );
  ...
  goNextQuickFolder: function () {
    let aFolder = QuickFolders.Util.CurrentFolder,
      ...
      firstOne = null;
    if (!aFolder) {
      // we are probably in search results
      return;
    }
    for (let i = 0; i < QuickFolders.Model.selectedFolders.length; i++) {
      let folderEntry = QuickFolders.Model.selectedFolders[i];
      ...
      if (found) {
        ...
        if (QuickFolders_MySelectFolder(folderEntry.uri)) {
          return true;
        }
      }
  ...
  goNextSiblingFolder: function () {
    ...
    if (iCurrent == childFolders.length - 1) {
      // go to first element
  ...
  displayNavigationToolbar: async function (optionsOrEvent) {
    function safeLoadScript(win, url) {
      try {
    ...
    try {
      ...
      const isCurrent =
        !isEvent &&
        !isFromWindow &&
        typeof optionsOrEvent.display == "boolean" &&
        !optionsOrEvent.doc; // toggle, from a dropdown menu
      ...
      util.logDebugOptional(
        "interface.currentFolderBar",
        `displayNavigationToolbar(visible=${isVisible}, selector=${selector})`
      );
      ...
      if (optionsOrEvent.display && !currentFolderBar) {
        if (!contentWin.QuickFolders_injectCurrentFolderBar) {
          ...
          if (cw && cw) {
            ...
            // messenger.WindowListener.loadScript
            safeLoadScript(cw, "chrome/content/scripts/qf-3pane.js");
  ...
  initToolbarTabListener: function () {
    ...
    if (tabmail) {
      let monitor = {
        onTabTitleChanged: function (_aTab) {},
        onTabSwitched: function (aTab, _aOldTab) {
          let tabMode = QuickFolders.Interface.CurrentTabMode;
          QuickFolders.Util.logDebugOptional("toolbarHiding", `tabMode = ${tabMode}`);
          if (tabMode.startsWith("mail")) {
            // issue 666
            QuickFolders.Interface.Toolbar?.removeAttribute("data-preview-mode");
          }
  ...
  onDeckChange: async function (targetTab) {
    ...
    if (targetTab && targetTab.selectedPanel) {
      ...
      util.logDebugOptional(
        "toolbarHiding",
        `onDeckChange - toolbar: ${toolbar.id} - panel: ${panelId}`
      );
      // mode = panelId;
    } else {
      //tab
      util.logDebugOptional(
        "toolbarHiding",
        `onDeckChange - toolbar: ${toolbar.id} - mode: ${mode}`
      );
    ...
    if (isMailPanel || isMailSingleMessageTab) {
      ...
      if (tabmail && tabmail.currentTabInfo) {
        ...
        if (info.QuickFolders_ToolbarStatus) {
          // only if it is stored for this tab, we set a value
    ...
    if (toggleButtonElement) {
      // manage checked status manually
      ...
      if (checkButton) {
        ...
        if (isButtonPressed) {
          // Tb resets the checked status afterwards ?
          setTimeout(() => {
            checkButton.setAttribute("aria-pressed", true);
            (toggleButtonElement.setAttribute("aria-pressed", true), 250);
          });
        }
      }
    }
  ...
  moveFolders: function (fromFolders, isCopy, targetFolder) {
    ...
    function isChildFolder(f) {
      for (let i = 0; i < fromFolders.length; i++) {
        ...
        let p = f;
        for (let ancestor = p.parent; ancestor; ancestor = ancestor.parent) {
          if (ancestor == fromFolders[i]) {
            return true;
          }
        }
      }
  ...
  showPalette: function (button) {
    ...
    QuickFolders.Util.logDebugOptional(
      "interface",
      `Interface.showPalette(${button.id}): context = ${context}`
    );
    ...
    if (paintButton) {
      ...
      if (active) {
        let tabColor = 1,
          folder = util.CurrentFolder;
        if (folder) {
          const folderEntry = QuickFolders.Model.getFolderEntry(folder.URI);
        ...
        try {
          ...
          menupopup.childNodes.forEach((e) => {
            if (e.tagName == "menuitem") {
              hasPopup = true;
            }
          });
          if (!hasPopup) {
            ...
            menupopup.insertBefore(
              this.createIconicElement("menuseparator", "*"),
              menupopup.firstChild
            );
        ...
        toolbar.style.setProperty(
          "cursor",
          "url(chrome://quickfolders/content/skin/ico/fugue-paint-cursor.png) 14 13, auto",
          "important"
        ); // supply hotspot coordinates
      } else {
  ...
  removeAnimations: function removeAnimations(styleSheetName) {
    ...
    styleEngine.removeElementStyle(ss, iconSelector, ["animation-name", "height", "width"]);
    styleEngine.setElementStyle(
      ss,
  ...
  showLicenseDialog: function showLicenseDialog(featureName) {
    ...
    win
      .openDialog(
        "chrome://quickfolders/content/register.xhtml",
        "quickfolders-register",
  ...
  quickMoveHelp: function (_el) {
    ...
    h.textContent = util
      .getBundleString("quickMove.help.head")
      .replace("{1}", "quickMove / quickJump");
    box.appendChild(h);
  ...
  showAboutConfig: function (clickedElement, filter, readOnly, updateUI = false) {
    ...
    if (updateUI) {
      ...
      w.addEventListener("unload", (_event) => {
        QuickFolders.Util.notifyTools.notifyBackground({
  ...
  pasteFolderEntriesFromClipboard: function () {
    ...
    try {
      ...
      Services.prompt.alert(
        null,
        "QuickFolders",
        QuickFolders.Util.getBundleString("qf.alert.pasteFolders.formatErr")
      );
    }
  },
  ...
  findRelated_Click: async function (event) {
    ...
    try {
      ...
      regexOption.searchSelected = JSON.parse(
        QuickFolders.Preferences.getStringPref("findRelated.searchSelected")
      );
      regexOption.searchCriteria = JSON.parse(
        QuickFolders.Preferences.getStringPref("findRelated.searchCriteria")
      );
  ...
  patchToolbarTheme: function (event, toolbarObj) {
    ...
    if (tabmail) {
      tabIndex = Array.from(tabmail.tabInfo).findIndex((t) => t.chromeWindow === toolbarObj.win);
    }
